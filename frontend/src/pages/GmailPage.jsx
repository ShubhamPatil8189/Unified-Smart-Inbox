  ...
  const fetchGmail = useCallback(async () => {
    ...
    try {
      const endpoint = searchQuery
        ? `/priority/user/${user.id}/search?q=${encodeURIComponent(searchQuery)}&provider=gmail`
        : `/priority/user/${user.id}/emails?provider=gmail`;
        ...
        <div className="col-span-2 space-y-6">
          <div className="rounded-2xl bg-white dark:bg-gray-800 p-6 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-lg font-semibold text-slate-900 dark:text-white">Gmail status</h2>
                <p className="mt-1 text-sm text-slate-500 dark:text-gray-400">{gmailConnected ? "Connected • Auto-refreshes every 2 min" : "Gmail not connected"}</p>
              </div>
            ...
            <div className="mt-6 space-y-4">
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center p-8 text-center bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
                  ...
                  <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-1">Connect Your Gmail</h3>
                  <p className="text-sm text-gray-500 dark:text-gray-400 max-w-sm mb-5">To view your emails here, please authorize access to your Google account.</p>
                  <button 
                    onClick={async () => {
                      try {
