...
const priorityRuleService = require("./priorityruleservice");
...
const SYSTEM_INSTRUCTIONS = `You are an email priority classifier and action extractor.
Return only a valid JSON object with this exact schema:
{
    ...
    "detected_action": {
        ...
        "location": "Meeting location (if event)",
        "attendees": ["list", "of", "names/emails"],
        "due_date": "YYYY-MM-DDTHH:mm:ssZ (for TASK)",
        "start_time": "YYYY-MM-DDTHH:mm:ssZ (for EVENT)",
        "end_time": "YYYY-MM-DDTHH:mm:ssZ (for EVENT)"
    } (optional, only if actionable)
}
Rules:
- Use the provided email content and context only.
- Detect tasks like "submit report by tomorrow" or events like "meeting on Friday at 3pm".
- priority_score and confidence must be numbers in [0, 1].
- Use the current date for context: ${new Date().toISOString()}.
...
function normalizeModelResult(parsed) {
    ...
    const result = {
        ...
        priority_score: clamp(rawScore || 0, 0, 1),
        confidence: clamp(rawConfidence || 0, 0, 1),
        reason
    };

    if (parsed.detected_action && typeof parsed.detected_action === "object") {
        const da = parsed.detected_action;
        if (da.type && da.title) {
            result.detected_action = {
                type: String(da.type).toUpperCase(),
                title: String(da.title).trim(),
                description: String(da.description || "").trim(),
                location: da.location ? String(da.location).trim() : null,
                attendees: Array.isArray(da.attendees) ? da.attendees.map(a => String(a).trim()) : [],
...
const priorityService = {
    ...
    async analyzeEmail(emailId, options = {}) {
        ...
        try {
            ...
            // 1. Check Custom Priority Rules first
            const ruleMatch = await priorityRuleService.matchEmailToRule(email);
            if (ruleMatch) {
                await priorityService.savePriority(email.id, ruleMatch);
                await processingService.markProcessingCompleted(email.id);
                return { success: true, emailId: email.id, result: ruleMatch };
            }
            ...
            // 2. Handle detected actions (tasks/events)
            if (result.detected_action) {
                // Break circular dependency by requiring taskService locally
                const taskService = require("./taskservice");
                await taskService.handleAIExtraction(email, result.detected_action);
            }
